...
(() => {
  ...
  function adminTemplate() {
    ...
    <div class="u-admin-grid">
      ...
      ${card(heading('Cerrar o limpiar evento','alert','Primero verifica el respaldo. Después se archiva el evento y se abre uno vacío; no se borra el historial.')+`<div class="u-actions">${button('Respaldar y cerrar evento','check','app.closeEventAndStartNext()','u-primary')}${button('Limpiar evento con respaldo','reset','app.clearEventWithoutSaving()')}</div><p class="u-muted">No hay borrado automático por inactividad. El reloj empieza con el primer pedido o con el botón de inicio; recargar no reinicia el servicio.</p>`,'u-full')}
    </div>`;
  }
